...
## Task 7 final scoped integration repair (2026-09-04)

The integration review identified four gaps not proved by the earlier 120-test suite. The production session observation now carries its existing version and original cleanup path. After validating the full receipt, the runner preserves a complete `cleanup.json` with a SHA256 reference and projects only the frozen ten fields into both scorer receipt slots. A real scoped fake-loader session completes all 25 synthetic fixture calls through the runner, and the frozen scorer receipt validator accepts both final slots without accessing Gold.

Cleanup has an independent 120-second monotonic deadline, rather than a two-second grace shorter than managed release and artifact hashing. Real subprocess tests cover a 2.3-second managed close and a short-budget timeout that remains unverified. Official GUI-Actor predictions without top-k points retain their full raw telemetry, become nonterminal malformed output, and allow the same loaded runtime to handle the next target. Session request paths bind to the verified session root independently of model storage; distinct-root mailbox calls and image/identity escape negative controls are covered. Each finding received a narrow RED/GREEN cycle. No scorer, canonical adapter, process-scope primitive, model acquisition, or live model execution is changed or claimed by this slice.

The final related six-file suite passed **127 tests in 95.41s**. This is synthetic/offline runtime-contract evidence, not model quality, Gold scoring, or actual-matrix completion.
