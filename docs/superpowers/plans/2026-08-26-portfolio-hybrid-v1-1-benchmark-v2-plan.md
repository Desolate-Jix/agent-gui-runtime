...
- v1七项文件、24 images、120 targets、12+12 screens、60+60 targets及partition membership不变。除Task5/6明确allowlist的`workflow_service.py`/`workflow_worker.py`及Task6对`app/api/panel.py`现有worker start/adopt/continue/cancel/heartbeat endpoint的同锁guard wiring外，只新增名字/contract含`v2`的文件；非benchmark请求/响应与incumbent/Hybrid行为必须byte/semantic不变。
...
| `app/learn/hybrid/benchmark_v2_worker_binding.py` | serialized binding在spawned worker内重验/安装；normal clear与supervisor abnormal cleanup receipts |
...
Narrow existing-file allowlist：`app/learn/workflow_service.py`（Task5 server-owned serialized binding injection；Task6 durable benchmark mode、shared operation mutation lock、production composition与complete/cancel atomic cut-point）、`app/learn/workflow_worker.py`（Task5 spawned-child install/normal-clear及supervisor abnormal-cleanup hook）和`app/api/panel.py`（Task6仅把现有worker start/adopt/continue/cancel/heartbeat endpoint内部改接同一guarded workflow-service functions；route/request/response shape不变，panel不得直接访问Registry）。不得修改其他existing production files。
...
Cleanup evidence区分两条路径：正常return、Python exception、cooperative cancel及可执行Python unwinding的`BaseException`必须在child `finally`退出pinned context、清除process-local binding并写`normal_clear_receipt`；`TerminateProcess`、outer worker death、power/crash不会执行Python `finally`，不得伪造clear receipt。强杀时由Task7A supervisor abnormal recovery按sealed operation lineage终止exact child PID/create-time及Job descendants，关闭process/Job handles，并在Task4独立reconcile HWND；只有`stable_zero_processes=true`、`job_members=0`、handles closed、HWND absent均有OS observation时才能写`abnormal_cleanup_receipt`并接受缺失的normal clear，否则fail closed。

- [ ] RED: parent-only in-memory binding在real spawned worker内得到`no_bound_window`；serialized canonical JSON在real spawned worker内安装成功且adopted receipt的payload SHA/capture SHA/UIA root HWND/owner PID等于owner seal；wrong PID/create-time/Job/HWND/capture/journal/payload SHA、ambient/multiple/stale binding在provider dispatch前FAIL；normal exit必须有normal clear；real强杀明确无finally receipt，但只有supervisor abnormal receipt + exact PID/create-time/Job/handle/HWND stable-zero后可接受。
...
**Cleanup:** test harness自身在`finally`关闭WorkflowService operation与test window；正常child断言normal clear，强杀child断言没有伪造normal clear且必须由supervisor/Job abnormal recovery形成stable-zero receipt。两路最终都要求child/descendants、HWND、handles、listener、lease全部不存在。
...
- Modify `app/learn/workflow_service.py` only for durable benchmark mode, shared per-operation mutation lock/CAS guard, production singleton composition, guarded generic start/adopt/continue/cancel/heartbeat mutators and complete/cancel terminal cut-points
- Modify `app/api/panel.py` only so its existing worker start/adopt/continue/cancel/heartbeat endpoints call the guarded workflow-service functions; no route/model/response change and no direct Registry access remains in those endpoints
...
class BenchmarkV2IncumbentWorkflowService(Protocol):
    def start_incumbent_observe(self, *, provider_case: Mapping[str, object],
...
这是versioned benchmark-scoped internal public-service lifecycle，不新增HTTP/panel/action API。`get_production_benchmark_v2_workflow_service()`在`workflow_service.py`内lazy-bind现有production singletons `app.learn.workflow_store.learning_workflow_run_store`与`app.learn.workflow_worker.learning_stage_worker_registry`；panel generic endpoints和benchmark façade必须使用这同一对objects。Runner/`benchmark_v2_actual.py`只能取得façade，不能import、构造或传递Registry/store，也不能创建平行composition。

Start以store expected-revision CAS把sealed `learning_pipeline_mode=benchmark_v2_incumbent_single_observe`、operation/worker/window lineage持久化后，才idempotently启动唯一`vision_observe_screen`。`workflow_service.py`为每个`(id(store),run_id,operation_id)`提供同一server operation mutation `RLock`；现有store owner lock阻止第二production store process，durable expected-revision CAS处理restart/replay。Lock order固定为operation mutation lock→store get/transition CAS→Registry status/adopt/start/cancel；不得反向。现有generic worker start/adopt/`continue_learning_stage_worker_result`/cancel/heartbeat、specialized terminalizer、restart reconciler都必须先取得此锁并从store读durable mode，不能相信caller payload。Panel现有五个endpoint只改内部调用到这些guarded functions；它们不得再直接调用Registry，nonbenchmark响应保持byte/semantic不变。只有`workflow_service.py`的production composition持有Registry/store。

对benchmark mode，所有generic/specialized adopt或continue入口都调用同一个`_resume_benchmark_v2_incumbent_terminalization`，绝不进入`interpret_learning_stage_worker_result`、`_ensure_next_managed_stage_operation`或`_start_next_managed_stage_worker`；后二者也有defense-in-depth mode guard，发现该mode即fail closed。Complete状态机固定：①锁内从Registry completed status验证exact worker/result SHA、binding receipt与lease/cleanup proof；②store expected-revision CAS写durable `terminal_intent`（含worker/result/cleanup refs）；③幂等Registry adopt/finalize exact result；④store CAS写唯一terminal `benchmark_v2_incumbent_observe_complete` receipt。Crash在①前重poll，①→②间无adopt，②→③间restart按intent adopt同一result，③→④间restart读取同一adopted receipt完成CAS，④后任何入口只返回byte-identical replay。

Guarded cancel使用互斥状态机：①同锁读取durable mode/revision；②仅在没有`terminal_intent`或terminal receipt时以CAS写`cancel_intent`（绑定operation/worker/PID-create-time/Job与reason）；③调用exact Registry cancel及Task7A supervisor termination；④取得normal或abnormal cleanup proof后CAS写唯一terminal `benchmark_v2_incumbent_cancelled` receipt。`terminal_intent`与`cancel_intent`由同一expected-revision CAS竞争，恰一winner；loser不得覆盖winner、重复cancel/adopt或改变outcome，只返回winner最终receipt或`intent_pending`。Crash在cancel intent前无mutation，intent→Registry cancel间由restart继续exact cancel，cancel→cleanup间等待同一incarnation stable-zero，cleanup→terminal CAS间由receipt refs恢复；所有重启均不能启动worker。Heartbeat也取得同一锁并读mode：只在纯`running`且无complete/cancel intent时按CAS续租同一operation；见任一intent或terminal receipt即拒绝且不得增加revision、覆盖evidence、重启worker或推进stage。任一intent/result/receipt漂移永久SAFE_STOP；complete/cancel/heartbeat任何路径都不能签发next worker或复活terminal operation。
...
- [ ] RED: 现有generic incumbent continuation负控会产生downstream worker；同一benchmark operation同时竞争panel generic start/adopt/continue/cancel/heartbeat五个入口，并在另一case加入specialized terminalizer作为第六对手，证明complete/cancel intent只有一个CAS winner、恰一terminal receipt、zero next worker/provider且heartbeat不能复活/续租terminal或推进Hybrid；在complete intent各点与cancel intent→Registry cancel→cleanup→terminal各点逐点crash并restart均返回winner的byte-identical replay；第二store/Registry composition、panel direct Registry access、stale revision、cleanup failure fail closed；旧panel incumbent与Hybrid的start/adopt/continue/cancel/heartbeat request/response及continuation graph保持不变。
- [ ] Run RED: `uv run pytest -q tests/test_portfolio_hybrid_v1_1_benchmark_v2_incumbent.py`。
- [ ] GREEN: 实现durable mode、shared lock/CAS、single production composition、panel五入口guarded wiring及recoverable complete/cancel/heartbeat state machine；generic incumbent/Hybrid行为保持原样。
...
**Independent review:** trace production singleton composition、start/adopt/continue/cancel/heartbeat与specialized/restart入口的同锁mode guard、complete/cancel intents互斥CAS、Registry adopt/cancel、cleanup、terminal各crash点与restart replay，并证明terminal不可被heartbeat/cancel复活且generic cascade不可达；输出`task-10b-slice-6-review.md`。

**No-action evidence:** operation只封存观察结果，不授权/执行下一stage、click或publish。

**Cleanup:** success/cancel/restart tests finally关闭single Qwen worker、lease、test window/service operation；zero downstream workers/providers/processes/listeners。
...
Production verifier只读server-owned Task7A owner/cleanup journals与raw sampler transcripts；不接收caller-supplied observation mapping。actual mode要求sealed production observer identity，fake/injected observer拒绝。Sampler保存下列命令的raw stdout/stderr/exit code/timestamp，并以OS observer补PID create-time：
...
External GPU fingerprint为非owned `(pid,create_time,gpu_uuid,memory)`集合；baseline/post窗口变化则`indeterminate`，不能填0。Owned VRAM按exact PID+create-time/device归属，device residual另报。Owner overlap从raw acquire/release intervals独立重算；Task7A receipt只是parent。
...
class WorkflowServicePort(Protocol):
    ...
    def start_incumbent_observe(self, *, provider_case: Mapping[str, object], window_binding: Mapping[str, object]) -> Mapping[str, object]: ...
    def poll_incumbent_observe(self, *, operation_ref: Mapping[str, object]) -> Mapping[str, object]: ...
...
**Cleanup:** 正常路径每screen finally关闭service/provider/window；强杀/outer death不声称执行child finally，改由Task7A supervisor abnormal recovery + Job cleanup + Task4 HWND reconciliation形成OS stable-zero receipt。结束运行Task9的`--cleanup-open-attempts` exact command并验证zero HWND/process/descendant/listener/lease/handle；失败BLOCK。
...
| C4 direct Registry / contaminated incumbent orchestration | CLOSED | Task6把durable `learning_pipeline_mode=benchmark_v2_incumbent_single_observe`写入现有store；specialized/restart与panel generic start/adopt/continue/cancel/heartbeat五入口共用同一operation mutation lock、store revision CAS与mode guard，panel不再直接访问Registry，`_ensure_next...`/`_start_next...`另有fail-closed guard。唯一production factory绑定现有store/Registry singletons。Complete `terminal_intent`与`cancel_intent`同revision互斥；winner按crash-safe Registry adopt或cancel→cleanup→terminal路径恢复，loser只重放同一receipt。Heartbeat只可续租无intent的running operation，不能覆盖intent、复活terminal或推进Hybrid；五方并发/逐点重启测试证明zero next worker/provider。 |
...
| I2 dependency order | CLOSED | DAG为Task0 estimand→Task1 isolation→Task2 predictions/scoring→Task3 durable claim→Task4 window owner→Task5 spawned binding→Task6 incumbent cut-point→Task7 lifecycle→Task8 actual adapter→Task9 runner/probes→Task10 seal tool→Task11 leakage/authorization code→Task12 report→Task13 final seal/regression→Task14 independent authorization→Task15 holdout→Task16 final scoring/report。 |
...
| Round3 I1 Task12 omitted load-bearing tests | CLOSED | Task12 GREEN exact command已加入`benchmark_v2_worker_binding.py`与`benchmark_v2_incumbent.py`两测试；release dependency manifest和review必须消费其真实PASS refs/source hashes。 |
| Round3 I2 forced termination was attributed to child finally | CLOSED | Task5/Task13区分normal clear与强杀：`TerminateProcess`/outer death预期无finally receipt，只在Task7A supervisor abnormal recovery、exact Job/PID/create-time stable-zero、handles closed及Task4 HWND absent后接受abnormal receipt。 |
| Round4 Important cancel/heartbeat bypassed the mutation authority | CLOSED | Global/Task6 allowlist已覆盖panel现有start/adopt/continue/cancel/heartbeat；workflow service定义同锁durable-mode guards、cancel intent状态机、heartbeat CAS规则及五方竞争和cancel crash/restart测试，同时保持所有非benchmark endpoint bytes/semantics不变。 |
