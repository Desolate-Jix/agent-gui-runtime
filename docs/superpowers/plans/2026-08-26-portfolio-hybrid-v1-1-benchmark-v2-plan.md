...
Start在同一operation `RLock`内先做active-operation/revision precheck，再调用amendment B1 `prepare_benchmark_worker_identity` reservation exact worker ID/nonce，调用B2 Registry acquisition-owner prepare只持久化model request intent/runtime owner且zero lease/process/listener/Job/provider，再以store expected-revision CAS持久化closed operation anchor、reservation、parent-independent expected supervision、acquisition refs、sealed `learning_pipeline_mode=benchmark_v2_incumbent_single_observe`与window lineage，随后调用`launch_prepared_benchmark_worker` exact once并做现有stage postcheck/必要compensation。Composition只持有production root/factory capability，绝不持有per-operation authority；actual supervision在launch时另绑定当前Registry parent identity，所以store-anchor后/launch前restart不要求remint worker/nonce/anchor。Crash/replay/cancel只能resume或abort同一reservation/owner，不得第二worker/nonce。`workflow_service.py`为每个`(id(store),run_id,operation_id)`提供同一server operation mutation `RLock`；现有store owner lock阻止第二production store process，durable expected-revision CAS处理restart/replay。普通lock order为operation mutation lock→store precheck/CAS→Registry；start是受控交错`precheck→Registry identity/owner prepare→store anchor CAS→Registry launch→postcheck/compensation`，每次Registry internal lock结束后才访问store；adopt严格`precheck→Registry`。现有generic worker start/status/adopt/`continue_learning_stage_worker_result`/cancel/heartbeat/finish/recover、specialized terminalizer、restart reconciler都必须先取得此锁并从store读durable mode，不能相信caller payload。Panel八个入口与read-only runtime attachment seam只改内部调用到这些guarded functions；endpoint不得再直接调用Registry，nonbenchmark响应、call order/count/side effects保持byte/semantic不变。只有`workflow_service.py`的production composition持有Registry/store/root factory。
...
- [ ] RED: 现有generic incumbent continuation负控会产生downstream worker；同一benchmark operation同时竞争panel generic start/status/adopt/continue/cancel/heartbeat/finish/recover八个入口，并在另一case加入specialized terminalizer，证明complete/cancel intent只有一个CAS winner、恰一terminal receipt、zero next worker/provider且heartbeat/finish/recover不能复活、覆盖winner或推进Hybrid；逐点覆盖prepare→anchor CAS→launch、complete intent和cancel的pre-gate/post-gate-pre-acquire/acquire-intent/lease-acquired→cleanup→terminal crash，restart均返回同一reservation/worker/receipt且zero replacement；same-identity reminted A snapshot必须被store-anchored Task5/B1/B2 parents拒绝；第二store/Registry/root composition、panel direct Registry method call、stale revision、missing acquisition intent、cleanup failure fail closed；逐入口断言旧panel incumbent与Hybrid完整request/response、exact call order/count/side effects及continuation graph不变，start为precheck→Registry→postcheck/compensation，adopt为precheck→Registry。
...
Release-gate dependency manifest必须按exact DAG列出Task5、A、B1、B2、C/Task6的独立PASS result refs与reviews：`task-10b-slice-5-review.md`、`task-10b-slice-6-prerequisite-a-review.md`、`task-10b-slice-6-prerequisite-b1-review.md`、`task-10b-slice-6-prerequisite-b2-review.md`、`task-10b-slice-6-review.md`。同时seal production source SHA：`benchmark_v2_worker_binding.py,workflow_worker.py,windows_process_scope.py,model_server.py,benchmark_v2_incumbent_operation.py,workflow_service.py,panel.py`；以及load-bearing test SHA/result refs：`test_portfolio_hybrid_v1_1_benchmark_v2_worker_binding.py,test_learning_workflow_stage_worker.py,test_learn_hybrid_windows_process_scope.py,test_model_request_cancellation.py,test_portfolio_hybrid_v1_1_benchmark_v2_incumbent.py,test_learning_workflow_stage_execution.py`。B1 direct ref必须覆盖reservation→store anchor→launch、same-live receipt retry、dead-supervisor inference及raw handle observations；B2 direct ref必须覆盖not-acquired/exact-exited disjoint outcomes；C ref必须覆盖start/adopt call order和combined remint rejection。缺失、非PASS、review有未关闭Critical/Important或任一SHA与final seal不一致均FAIL；C broad E2E不能替代A/B direct result refs。
...
uv run pytest -q tests/test_learning_workflow_stage_worker.py -k "completed_result_identity or benchmark_worker or prepared_benchmark_worker or benchmark_provider_cleanup"
uv run pytest -q tests/test_learn_hybrid_windows_process_scope.py -k "benchmark_worker or exact_process_identity_to_scope or handle_close_observation"
...
| Amendment I2 worker Job authority/name under-specified | CLOSED | B1冻结`benchmark_worker_supervision_v1`、closed operation anchor、production/test disjoint root capability、prepare reservation→store expected supervision→launch协议、execution nonce及`benchmark_worker_scope_name_v1`公式；composition不持有per-operation authority，fresh observer从store anchor验证expected supervision，outer-worker namespace与provider grammar互不相容。 |
