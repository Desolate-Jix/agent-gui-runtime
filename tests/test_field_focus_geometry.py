...
def test_recognition_does_not_refine_a_field_into_the_address_bar(tmp_path, monkeypatch, uia_ready):
    ...
    goal = "Click the Google search input field at the top of the results page"
    ...
    assert len(calls) == 1
    chosen = response.data["result"]["recommended_target"]
    assert chosen["element"]["click_point"] == {"x": 496, "y": 140}
