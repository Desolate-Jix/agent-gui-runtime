...
def test_type_text_verifies_clipboard_before_paste_and_restores_after_settle(monkeypatch) -> None:
    from test_text_input_guard import prepared
    ...
    result = controller.type_text("list comprehension", x=70, y=80, click_before_typing=True, clear_existing=True)
    ...
    assert events.index("clipboard-verified") < events.index((VK_CONTROL, VK_A))
    assert events.index((VK_CONTROL, VK_V)) < events.index(("sleep", input_module.CLIPBOARD_PASTE_SETTLE_SECONDS))
...
def test_type_text_fails_when_clipboard_write_verification_mismatches(monkeypatch) -> None:
    ...
    controller, _, events, session = prepared(monkeypatch)

    def mismatch():
        raise RuntimeError("Clipboard write verification failed before paste")

    monkeypatch.setattr(session, "verify_current_text", mismatch)
    with pytest.raises(RuntimeError, match="Clipboard write verification failed before paste"):
