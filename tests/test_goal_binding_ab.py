...
def test_arm_runner_never_calls_omni_and_verifies_snapshot_first(tmp_path: Path) -> None:
    ...
    calls: list[Path] = []
    arm = _arm(call=lambda image, _request: calls.append(image) or [0.2, 0.375], adapt=_native_adapter)
...
def test_arm_runner_calls_binder_once_for_each_of_25_goals(tmp_path: Path) -> None:
    ...
    arm = _arm(call=lambda image, request: calls.append((image, request)) or [0.2, 0.375], adapt=_native_adapter)
...
def test_cleanup_failure_blocks_arm_finalization_and_next_model(tmp_path: Path) -> None:
    ...
    arm = _arm(call=lambda _image, _request: [0.2, 0.375], adapt=_native_adapter, cleanup=lambda: failed)
...
def test_runner_runs_cleanup_after_all_binders_and_before_any_vista(tmp_path: Path) -> None:
    events: list[str] = []
    arm = _arm(
        call=lambda _image, _request: events.append("binder") or [0.2, 0.375],
...
def test_cleanup_failure_prevents_all_vista_dispatch(tmp_path: Path) -> None:
    vista_calls: list[str] = []
    arm = _arm(
        call=lambda _image, _request: [0.2, 0.375],
        adapt=_native_adapter,
...
def test_mutating_adapter_copy_or_forging_a_bound_candidate_cannot_change_authority(tmp_path: Path) -> None:
    ...
    arm = _arm(call=lambda _image, _request: [0.2, 0.375], adapt=malicious)
    artifact = _run(tmp_path, arm=arm, vista=lambda _image, target: vista_calls.append(target) or "[500,500]")
    ...
    forged_arm = _arm(call=lambda _image, _request: [0.2, 0.375], adapt=forged)
...
def test_cleanup_provider_must_match_arm_provider(tmp_path: Path) -> None:
    arm = _arm(call=lambda _image, _request: [0.2, 0.375], adapt=_native_adapter, cleanup=lambda: _clean("wrong-provider"))
...
def test_forged_native_unbound_against_removed_candidate_is_schema_invalid(tmp_path: Path) -> None:
    ...
    arm = _arm(call=lambda _image, _request: [0.2, 0.375], adapt=forged)
    artifact = _run(tmp_path, arm=arm)
    assert artifact.metrics["binder"]["schema_invalid"] == 25
    assert artifact.metrics["abstained"] == 25


def test_runner_preserves_closed_native_envelope_and_adapts_only_projection(tmp_path: Path) -> None:
    from hashlib import sha256
    raw = "[0.2,0.375]"
    envelope = {
        "contract_version": "goal_binding_native_trace_v1",
        "profile_identity": {"profile_id": "ui", "preprocessing_sha256": "a" * 64, "runtime_sha256": "b" * 64, "native_output_kind": "ui_venus_point_v1"},
        "raw_native_output": raw, "raw_native_output_sha256": sha256(raw.encode("utf-8")).hexdigest(),
        "parsed_native": [0.2,0.375], "resource_metrics": {"latency_ms": 1.0, "peak_vram_bytes": 0},
        "worker_process_identity": {"pid": 42, "create_time_ns": 99},
        "request_lineage": {"capture_id": "capture/x", "screenshot_sha256": "c" * 64},
    }
    artifact = _run(tmp_path, arm=_arm(call=lambda _image, _request: envelope, adapt=_native_adapter))
    binder = next(entry for entry in artifact.cases[0]["trace"] if entry["slot"] == "binder")
    assert binder["native_raw"] == raw
    assert binder["native_envelope"] == envelope
    assert binder["native_parsed"]["point"] == [0.2, 0.375]
