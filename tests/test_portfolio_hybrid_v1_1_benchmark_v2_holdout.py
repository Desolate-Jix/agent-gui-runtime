...
def _launch(scope_name: str, work: Path, value, payload, *, mode="claim", failpoint=None, test_control=None, event=None, authorization_ref=None):
    token = uuid.uuid4().hex
    ...
    try:
        ...
        details = stderr_path.read_text(encoding="utf-8", errors="replace")
        raise AssertionError(f"child readiness failed: {details}") from error
...
def test_authorization_publication_waits_past_create_before_write(test_backend, tmp_path: Path) -> None:
    ...
    try:
        ...
        processes.append(creator)
        creator_start.write_text("start", encoding="utf-8")
        _wait_for(marker)
        loser, loser_start, loser_result = _launch(name, work, test_backend, payload)
...
def test_two_job_contained_processes_have_one_winner_and_same_attempt(test_backend, tmp_path: Path) -> None:
    ...
    processes = []
    try:
        launched = [_launch(name, work, test_backend, payload) for _ in range(2)]
        processes = [item[0] for item in launched]
...
def test_each_crash_phase_has_exact_recovery_state_and_job_cleanup(
    ...
    processes = []
    try:
        process, start, _ = _launch(name, work, value, payload, failpoint=failpoint)
...
def test_concurrent_chain_append_has_no_sibling_records(test_backend, tmp_path: Path) -> None:
    ...
    try:
        launched = [
            _launch(name, work, test_backend, {}, mode="append", event=event)
            for _ in range(2)
        ]
        processes = [item[0] for item in launched]
        for _, start, _ in launched:
...
def test_concurrent_recovery_mirrors_one_claim_record(test_backend, tmp_path: Path) -> None:
    ...
    try:
        launched = [
            _launch(name, work, test_backend, payload, mode="recover") for _ in range(2)
        ]
        processes = [item[0] for item in launched]
        for _, start, _ in launched:
            start.write_text("start", encoding="utf-8")
        ...
        assert [item["state"] for item in results] == ["consumed", "consumed"]
