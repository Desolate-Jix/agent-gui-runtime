...
def test_windows_initial_empty_zero_sequence_can_type_and_restore():
    api = FakeWin32Api({}, sequence=0)
    with ClipboardTextTransaction("fresh", backend=build_windows_backend(api)) as transaction:
        assert transaction.verify_current_text() == 1
        assert api.current_bytes(CF_UNICODETEXT).startswith("fresh".encode("utf-16-le"))
    assert transaction.status == "restored"
    assert api.formats == {}
    assert not api.opened and not api.created_windows and not api.live_hglobals


def test_windows_zero_sequence_with_existing_data_is_not_accepted():
    api = FakeWin32Api({CF_UNICODETEXT: b"x\x00\x00\x00"}, sequence=0)
    with pytest.raises(ClipboardTransactionError, match="clipboard_sequence_unavailable"):
        with ClipboardTextTransaction("fresh", backend=build_windows_backend(api)):
            pytest.fail("must not write")
    assert "empty" not in api.events


def test_windows_initial_zero_sequence_does_not_overwrite_concurrent_copy():
    api = FakeWin32Api({}, sequence=0)
    backend = build_windows_backend(api)
    snapshot = backend.capture()
    api.external_copy("newer")
    try:
        with pytest.raises(ClipboardTransactionError, match="ownership_conflict"):
            backend.replace_with_text("fresh", snapshot.sequence, snapshot=snapshot)
        assert api.current_bytes(CF_UNICODETEXT).startswith("newer".encode("utf-16-le"))
        assert "empty" not in api.events
    finally:
...
@pytest.mark.parametrize("fault", ["not_locked", "enumeration_failed", "nonempty"])
def test_windows_initial_zero_requires_successful_locked_empty_enumeration(fault):
    api = FakeWin32Api({}, sequence=0)
    backend = build_windows_backend(api)
    if fault == "not_locked":
        with pytest.raises(ClipboardTransactionError, match="clipboard_sequence_unavailable"):
            backend._require_sequence(api, allow_initial_empty=True)
        return
    try:
        with backend._clipboard_lock(api):
            if fault == "enumeration_failed":
                api.fail_enum = True
            else:
                api.formats[CF_UNICODETEXT] = api._seed(CF_UNICODETEXT, b"x\x00\x00\x00")
            with pytest.raises(ClipboardTransactionError, match="clipboard_sequence_unavailable"):
                backend._require_sequence(api, allow_initial_empty=True)
        assert "empty" not in api.events
    finally:
