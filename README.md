...
3. 现有面板已提供明确的 Compile → CAS Publish → read-only Preview 控件：操作绑定已保存且 SHA 精确匹配的工作流，并要求 current observation；工作流编辑或切换会使状态失效，且不授予 action/capture 授权。下一步是 synthetic SEEK E2E；v2 当前仍不覆盖 `read`/`scroll` 的可执行回放。
