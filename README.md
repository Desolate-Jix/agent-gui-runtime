...
> Implementation status: `actual` is now technically wired, but remains unreachable unless both `--mode actual` and `--operator-approved-model-start` are supplied. Construction is lazy and starts nothing. Once explicitly invoked, the diagnostic holds at most one GPU provider: five managed Omni native workers share one phase scope, five compact Qwen ordinal calls share one exact scoped lease, and at most 25 bare-point VISTA calls share one exact lease. Each phase projects existing exact release/scope evidence into the closed cleanup observation and blocks the next provider on dispatch or cleanup uncertainty. This implementation work did not run `actual` or start a model; no fresh accuracy claim is made.
