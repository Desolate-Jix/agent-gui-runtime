# Project Iteration History Archive

This file preserves the detailed README history before the 2026-07-15 documentation simplification. For installation, current evidence, and the concise project timeline, start from [`README.md`](README.md).

## Archived README Content

## 2026-07-15 Learning Mode phase acceptance

The current read-only Learning Mode phase has passed with declared limitations. The phase report is `logs/benchmarks/learning_mode_stage_acceptance_20260715/stage_acceptance_report.json` and records `phase_acceptance_passed_with_declared_limitations`. Four current real-interface evidence sets cover Apple Music, Python.org, Windows Settings, and Steam; every review compares the source screenshot, Stage1 structure image, and final fused image.

The protected recursive rerun at `logs/benchmarks/learning_interface_chain_stage_acceptance_20260715_rerun/learning_interface_chain_smoke_report.json` contains nine checksum-pinned cases, nine complete three-image audits, nine completed read-only chains, eight `review_only_chain_ready` cases, and one Python.org `stress_only_needs_review` case. Current panel presentation evidence at `logs/benchmarks/learning_mode_stage_acceptance_20260715/learning_interface_presentation_evidence_v1.json` covers desktop, narrow layout, resizable map/details, the current fused overlay, non-empty page details, and a read-only PathGraph preview.

This is not final project acceptance. The strict readiness report at `logs/benchmarks/learning_mode_stage_acceptance_20260715/learning_mode_demo_goal_readiness_report.json` keeps `display_demo_ready=false` and `final_goal_complete=false` because pure-model/model-only template readiness is not proven. Fresh repository verification reports `1538 passed`; API/model services were stopped and ports `8000`, `1234`, `1240`, `1241`, `1244`, and `1245` were confirmed closed. No live click, fill, submit, Execute binding, live safe fill, or Runtime PathGraph promotion occurred.

## 2026-07-15 Learning Interface compact-row ROI checkpoint

The learning calibration chain now uses a compact, parent-clipped ROI for `list_row`, `table_row`, and `file_row` candidates. The root failure was visible on Python.org: a left-column news row was expanded to twice its width, so the VISTA crop included the parallel right column and returned a point there. The common fix keeps dense document/file rows local to their own column while deliberately leaving `conversation_row` behavior unchanged.

The checksum-pinned Python.org rerun moved VISTA point evidence from `24 inside / 1 outside` to `25 inside / 0 outside`, and precise review passes from `7` to `8` in the single-case rerun. The protected nine-case recursion was then rerun at `logs/benchmarks/learning_interface_chain_compact_roi_regression_20260715/learning_interface_chain_smoke_report.json`: all nine cases retain complete original/Stage1/final image sets, completed read-only chains, passed class/profile contamination audits, and zero VISTA points outside their candidate boxes. File Explorer rows remain in their own list column; Steam and WhatsApp conversation rows retain their previous wider context policy.

Verification: `uv run pytest tests/test_vision_route.py tests/test_vision_observe_locate.py -q` reports `101 passed`; `uv run pytest tests -q` reports `1533 passed`; Python compilation passes. VISTA and the local API were stopped after verification, with ports `1244` and `8000` confirmed closed. This is fixed-sample point-containment and regression evidence, not a general recognition-accuracy result. Python.org remains a dense-document stress sample because many candidates still require Gate review.

No target-app click, fill, submit, Execute binding, live safe fill, or Runtime PathGraph promotion occurred.

## 2026-07-15 Learning Interface final recursive checkpoint

The real Learning Interface panel has now completed the full read-only flow on WhatsApp and Steam: bind/capture, Qwen 8B full-screen understanding, numbered map, page details, OCR + VISTA + rerank + Gate dry-run, fusion, and read-only PathGraph preview. The panel displayed the current fused overlay rather than the raw screenshot, loaded non-empty page details, rendered the PathGraph preview, and allowed the map/detail split to be resized. A Steam run spent real time in precise calibration instead of skipping the step: `167` candidates entered the eligibility gate, `45` were eligible for grounding, `122` were blocked, `41` grounding results were valid candidates, and `4` were rejected. This is process evidence, not a grounding-accuracy claim.

The grounding eligibility contract was tightened in the common layer. Geometry-only evidence such as `calibrated_target_validated` or `cross_evidence_overlap` no longer upgrades `text`, `section`, or `group` items into actionable grounding candidates. Those roles require direct interaction evidence such as UIA invoke/value/edit, DOM click/edit, OmniParser interaction, or a ranked Execute candidate. The replayed Steam artifact now reports `non_actionable_leaked_to_grounding.leaked_count=0`.

The checksum-pinned recursive benchmark was rerun after this change:
...
uv run python scripts\run_learning_interface_chain_smoke.py `
  ...
  --out logs\benchmarks\learning_interface_chain_goal_final_20260715 `
  --json
```

The report has `9/9` complete read-only chains, `9/9` same-source original/Stage1/final evidence sets, and `9/9` class-rule contamination audits passing. Steam (two fixtures) and WhatsApp (one fixture) independently classify as `conversation_workspace` and use `conversation_rows`, covering the same class across different software and geometry. Eight cases are `review_only_chain_ready`; Python.org remains `stress_only_needs_review` because its dense two-column document layout still needs human review. Report: `logs/benchmarks/learning_interface_chain_goal_final_20260715/learning_interface_chain_smoke_report.json`.

Final verification: `uv run pytest tests/test_vision_route.py -q` reports `35 passed`; `uv run pytest tests -q` reports `1531 passed`. The VISTA model service and local panel API were stopped after verification, with ports `1244` and `8000` confirmed closed. These results verify the current fixed workflows and regression suite; they are not a general recognition-accuracy claim.

For the real panel check, two external Chrome extension sessions disconnected at the native bridge boundary. Following the documented browser fallback rule, the local panel was verified in the Codex in-app browser while the target application remained an external Windows window. A QQ run separately safe-stopped when its bound HWND became stale; that failure was not hidden by a fallback. No target-app click, fill, submit, Execute binding, live safe fill, or Runtime PathGraph promotion occurred.

## 2026-07-13 Current-presentation acceptance gate

Final Learning Mode readiness now requires a separate `learning_interface_presentation_acceptance_v1` gate. The panel renders this gate beside model-chain readiness, and learning-draft history exposes its same-source three-image, frontend-revision, desktop, narrow-layout, and blocker status. The top summary keeps requirement coverage separate from presentation coverage and shows `presentation_status` plus its blocker count. Recommended-source selection now requires `presentation_accepted=true`, so an older report cannot be treated as final merely because it once recorded model-chain completion.

The current report is `logs/benchmarks/learning_mode_demo_readiness_current_presentation_gate_20260713/learning_mode_demo_goal_readiness_report.json`. Its status is `display_demo_ready_presentation_unverified`; presentation evidence is `not_covered`, and final completion is false. This checkpoint was verified offline only. No Calculator, model, panel server, or real window was started while the user was gaming.

Presentation evidence can be supplied without editing the source scaffold:

```powershell
uv run python scripts\report_learning_mode_demo_goal_readiness.py `
  --scaffold logs\benchmarks\learn_fresh_model_demo_scaffold_v2_20260706\learn_mode_demo_scaffold.json `
  --presentation-evidence logs\benchmarks\<current-demo>\learning_interface_presentation_evidence_v1.json `
  --out logs\benchmarks\<current-demo-readiness>
```

The external evidence file overrides only the in-memory audit input. The scaffold remains unchanged, and the report records `source_presentation_evidence_path` for replay.

When presentation evidence is missing, the `capture_current_presentation_evidence` next action includes a non-evidence `evidence_template`. It precomputes the current `panel.js` and `panel.css` hashes while leaving all artifact paths, viewport dimensions, and visual-verification flags empty or false. The template cannot satisfy the gate until real evidence replaces those placeholders.

## 2026-07-13 Latest-fusion and foreground-activation correction

The Learning Interface panel now keeps the newest fusion artifact throughout the complete dataflow. A fresh `two_stage_understanding.fusion.compiled_overlay_path` is no longer overwritten by an older attached report, historical draft loading prefers that current nested fusion status, and the browser flow no longer renders the stale pre-fusion image again after loading the final draft. Focused regression coverage verifies all three paths.

Generic window activation now temporarily attaches the service thread to the foreground and target input threads before activation, then detaches them after the attempt. The existing exact-HWND foreground check remains mandatory. This change has passed the focused offline window-manager tests, but a new real-window verification is still pending because the user paused GUI/model testing while gaming.

The recent Calculator learning run is explicitly **not** a recognition or demo success: Stage1 blocked before Stage2 numbering, calibration accepted only 5 review candidates, and the draft produced 0 actions. Calculator testing is paused and must not be resumed without an explicit user request. No local model or panel server is currently running. The latest changed-path verification is `93 passed` plus Python syntax compilation; the full repository suite must be rerun before final demo acceptance.

## 2026-07-13 General UI hierarchy and real-panel checkpoint

Learning Mode now emits a display-only `ui_hierarchy_graph_v1` after Stage2. The graph records `screen -> structure_region -> section -> component_group -> component -> content`, explicit parent/child edges, source evidence, review state, and hierarchy validation. A shared ownership resolver arbitrates competing semantic groups before the graph is built; ancestor aggregation is retained, rejected leaf claims remain auditable, and ambiguous ties require review. The result also creates a hierarchy-backed `learning_template_draft_v1` plus `learning_draft_page_details_v1`, so the panel can display a collapsible UI hierarchy beside the read-only PathGraph preview. None of these assets authorize Execute or Runtime PathGraph promotion.

The fixed benchmark command is `uv run python scripts/run_general_ui_recognition_benchmark.py --manifest artifacts/benchmarks/general_ui_recognition_manifest_v1.json --out logs/benchmarks/general_ui_recognition_showcase_v25_20260713 --json`. The active manifest now has 11 valid checksum-pinned cases: 10 supported fixed recorded surfaces across 7 supported families plus one explicit NVIDIA hierarchy-review limitation. The ownership holdout has 30 human-curated owner-role annotations across 8 families. The latest report has no invalid fixture and no supported-case failure, but it deliberately reports `reliability_status=insufficient_application_diversity`; these are fixed recorded-surface checks, not model accuracy or general UI reliability.

The real panel flow now binds by exact window handle and verifies the returned handle before capture. Learn/observe normalizes legacy `provider_mode=local` to `local_understanding`, while precise calibration uses `local_grounding`. Learning capture retries low-information startup surfaces, and precise calibration displays model-loading / OCR+VISTA+rerank+gate progress with elapsed seconds instead of appearing frozen. A real Notepad run completed with a fused overlay, page details, and `UIHierarchyGraph` summary (`nodes=13`, no ambiguous/orphan/duplicate-owner/outside-parent nodes). No live click, fill, submit, Execute binding, or Runtime PathGraph promotion occurred. Mobile viewport evidence remains not covered because the external Edge viewport override did not take effect.

Benchmark-referenced screenshots are now protected from rolling screenshot cleanup. Two already-deleted historical fixtures were moved to `artifacts/benchmarks/general_ui_fixture_repair_backlog_v1.json`; one Apple Music screenshot was restored by checksum from an existing artifact. Invalid or missing fixtures remain excluded from the active denominator.
...
Learning captures reject low-information startup/splash surfaces before model inference. The readiness check uses distributed visual information across the window rather than trusting a title bar or a single center icon, and foreground activation must resolve back to the exact bound HWND (or its rooted child) before capture succeeds.

The app catalog includes Windows Calculator through `calc.exe`. Because Calculator is a UWP app whose top-level window is hosted by `ApplicationFrameHost.exe`, catalog binding requires a matching localized title (`Calculator` or `计算器`) and never falls back to an arbitrary shared-host window such as Settings.
