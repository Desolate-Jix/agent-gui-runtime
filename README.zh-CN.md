...
三个协议保持独立：Omni 只输出 `{bbox,type,content,interactivity}`；Qwen 在本地保留完整 runtime request，模型只看 ordinal `{i,box,active}` candidates；VISTA 接收新持久化的 candidate crop，且只返回 bare normalized `[x,y]`。runtime ID、capture lineage、crop hash、坐标变换、metrics 和 review-only 字段均由 adapter 持有。每次 provider 调用前后都重新验证 capture SHA 与尺寸。VISTA 受 25 个严格解析的 provider goal 限定：每个 goal 恰好产生一个 selected 或 abstained outcome，且只有唯一精确 role + label 的 eligible binding 可以 dispatch。provider 阶段固定批处理为 `Omni -> cleanup -> Qwen -> cleanup -> VISTA -> cleanup`；每个已开始阶段都在 `finally` 路径中恰好 release 一次，包括 provider 抛出意外异常时。cleanup 使用封闭 observation，owned/provider/helper/descendant process、listener 与 lease 均须为空，未验证 clean 时不得进入下一阶段；provider 与 cleanup 同时失败时保留显式 exception chain。scorer 先验证五个有序 case 和 25 个 outcome，然后才打开 Gold；regression report 同时记录封存 provider artifact 的 SHA-256。


> 实现状态：`actual` 已在技术上接通，但只有同时提供 `--mode actual` 和 `--operator-approved-model-start` 才能进入。构造过程为惰性且不启动模型。明确调用后，诊断路径最多只让一个 GPU provider 存活：5 个 managed Omni native worker 共用一个阶段 scope，5 个 compact Qwen ordinal 请求共用一个 exact scoped lease，VISTA 最多 25 个 bare-point 请求共用一个 exact lease。每个阶段从现有 exact release/scope 证据投影出 closed cleanup observation；dispatch 或 cleanup 不确定时会阻止下一 provider。本次实现工作没有运行 `actual` 或启动模型，也不产生新的准确率结论。
