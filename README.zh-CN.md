...
首轮完整真实推理基线在这些 regression 截图上得到 **5 正确 / 19 错绑 / 1 弃权**，
未通过 hard gate。25/25 原始响应为有效 JSON，其中一次选择 inactive candidate 被拒绝。
**替代模型 A/B 矩阵尚未完成，没有选出胜者**；这不是 live desktop 操作证明。实验不授予 GUI 执行权限，
不使用 unique holdout。新增模型测试工件受外部存储 **30 GiB** 上限约束，
原有 incumbent 资产保持只读。参见[模型获取边界](docs/GOAL_BINDING_MODEL_ACQUISITION.md)及
[基线结果与限制](docs/GOAL_BINDING_BASELINE_2026_09_04.md)。
...
> Phase A 历史实现说明（最新实测见上方）：`actual` 已在技术上接通，但只有同时提供 `--mode actual` 和 `--operator-approved-model-start` 才能进入。构造过程为惰性且不启动模型。明确调用后，诊断路径最多只让一个 GPU provider 存活：5 个 managed Omni native worker 共用一个阶段 scope，5 个 compact Qwen per-goal binding 请求共用一个 exact scoped lease，VISTA 最多 25 个 bare-point 请求共用一个 exact lease。每个阶段从现有 exact release/scope 证据投影出 closed cleanup observation；dispatch 或 cleanup 不确定时会阻止下一 provider。当时的实现 slice 没有运行 `actual` 或启动模型，也不产生新的准确率结论。
