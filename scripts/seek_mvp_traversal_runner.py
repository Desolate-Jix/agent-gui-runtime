...
DEFAULT_SEEK_URL = "https://www.seek.co.nz/software-engineer-jobs/in-All-Auckland"
...
def _execute_apply_entry(
    ...
    *,
    ...
    title = pre_apply_verification.get("observed_title") or detail.get("title") or job.get("title") or "the selected job"
...
def _verify_pre_apply_job_detail(
    ...
    *,
    ...
    company_matches = _titles_match(expected_company, observed_company) if expected_company else True
...
def _read_detail_until_complete(
    ...
    *,
    ...
    return {
        ...
        "detail_slices": slices,
        "completeness": completeness
        or assess_seek_job_detail_completeness(merged, scroll_count=max_scrolls, max_scrolls=max_scrolls),
        "scrolls": scrolls,
    }
...
def _verify_expected_value_from_structured_inventory(
    ...
    *,
    ...
    for visible in _iter_inventory_items(observation):
        ...
        same_field = bool(target_id and visible_id == target_id) or bool(target_label and target_label in visible_label)
        if not same_field:
            continue
        ...
        if dom_value and (dom_value == expected or expected in dom_value):
            ...
            ocr_match = True
            entry["matched_by"] = "ocr_text_secondary"
        matched_items.append(entry)
    primary_match = dom_match or uia_match
    failure_reason = None
    ...
    return {
        ...
        "field_relocation": {
            ...
            "matched_by": [item.get("matched_by") for item in matched_items if item.get("matched_by")],
        ...
        "verification_methods": {
            ...
            "ocr_near_field": {"available": bool(matched_items), "matched": ocr_match, "used_as_primary": False},
        },
    }


def _iter_inventory_items(observation: dict[str, Any]) -> list[dict[str, Any]]:
    inventory = observation.get("screen_inventory") if isinstance(observation.get("screen_inventory"), dict) else {}
    ...
    while (len(opened_details) if execute_clicks else len(processed_jobs)) < target_jobs and len(processed_jobs) < max_attempts:
        if not job_queue:
            if result_scroll_count >= max_results_scrolls:
                break
            scroll = _scroll_results_list(base_url, timeout=timeout, learned_artifact=learned_artifact)
    ...
    report.update(
        {
            ...
            "jobs_fully_read": sum(
                ...
                and assess_seek_job_detail_completeness(item.get("detail"), scroll_count=max_detail_scrolls)["complete"]
            ),
            "match_decisions": match_decisions,
