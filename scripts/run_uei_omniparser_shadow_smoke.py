...
def _runtime(root: Path, image: Path, size: dict[str, int]):
    ...
    manifest = _put(store, {"contract_version":"provider_manifest_v1","manifest_id":"manifest/smoke","provider_id":adapter.provider_id,"provider_version":adapter.provider_version,"profiles":[{"profile_id":adapter.profile_id,"operation":"screen_parse","input_contract":"screen_parse_request_v1","output_contract":"provider_safe_result_v1","declared_output_kinds":["element","text","icon","structure"],"supported_coordinate_spaces":["capture_pixel_xyxy"],"supports_capture_artifact":True,"privacy_capabilities":["restricted"],"mode_allowlist":["Shadow"]}]})
