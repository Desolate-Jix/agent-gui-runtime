...
def _build_capture(root: Path) -> tuple[Path, dict[str, Any]]:
...
def _hybrid_projection(
    ...
    *,
    ...
    trace: list[str] = []
    image_path, bundle = _build_capture(root)
...
def _panel_http_boundary(
    ...
    *,
    ...
    if workflow_state is not None:
        panel_api.learning_workflow_run_store = _StaticWorkflowStore(workflow_state)
    try:
...
def _save_large_review(
    ...
    *,
    ...
    state = _workflow_state(run_id=run_id, revision=revision, trial_path=relative_trial)
    ...
    with _panel_http_boundary(root, workflow_state=state) as client:
...
def _persist_approved_workflow_via_api(
    ...
    with _panel_http_boundary(root) as client:
        first = _post_success(
            client,
...
def build_managed_hybrid_review_source(
    ...
    *,
    ...
    """经 fake provider 边界和 Task 8 Large Review API 生成真实保存源。"""

    root = Path(project_root).resolve()
    root.mkdir(parents=True, exist_ok=True)
    image_path, projection, trace = _hybrid_projection(
        root,
        omni_runner=omni_runner,
        qwen_runner=qwen_runner,
        vista_runner=vista_runner,
    )
    large_review = _save_large_review(
        root,
        image_path=image_path,
    ...
    persisted = _persist_approved_workflow_via_api(root, workflow)
    ...
    return {
        **persisted,
        "provider_boundary_trace": trace,
        "large_review_save": {
            key: value
            for key, value in large_review.items()
            if key != "reviewed_candidate"
        },
    }
...
def _cas_snapshot(root: Path) -> dict[str, Any]:
    registry_path = (
        root / "runtime_state" / "reviewed-workflow-assets-v2" / "registry.json"
    )
...
def run_managed_two_process_persistence_proof(
    ...
    *,
    ...
    compiler_source_references_reviewed = reviewed_relative in json.dumps(
        compiler_review, ensure_ascii=False
    )
    ...
    workflow_store_path = Path(managed["large_review_save"]["workflow_store_path"])
    compile_payload = {
    ...
    cas_before_a = _cas_snapshot(root)
    ...
    try:
        ...
        cas_after_a = _cas_snapshot(root)
        ...
        cas_before_publish = _cas_snapshot(root)
        publish_payload = {
            **compile_payload,
        ...
        cas_after_publish = _cas_snapshot(root)
        duplicate_publish = _http_api_data(
            server_b["base_url"],
            "/panel/publish_reviewed_workflow_asset",
            publish_payload,
        )
        cas_after_duplicate = _cas_snapshot(root)
...
def run_fixture_negative_control(
    *, project_root: str | Path, fixture_path: str | Path
    ...
    workflow = _persist_approved_workflow_via_api(root, raw)
