...
def _safe_diagnostics(value, *, include_option=True, include_readiness=True):
    ...
    for key in ("scan_started", "scan_complete", "graph_scan_complete", "provider_tree_valid"):
        if type(value.get(key)) is bool:
            result[key] = value[key]
    if value.get("viewport_source") in {"native_owned_popup", "bound_window"}:
        result["viewport_source"] = value["viewport_source"]
...
def _describe(node, handle, pid, window, *, allow_outside_window=False):
    info = node.element_info
    element = info.element
    ...
    if bbox is None and allow_outside_window:
        # 展开选项先验证原始几何，完整扫描后再按已证明的弹窗视口筛选；不裁剪猜点。
        screen = _bound_rect(info.rectangle)
        if _rect_valid(screen):
...
def _associate_option_popups(options, *, handle, pid, window, identity, diagnostics=None):
    ...
    for option in options:
        ...
        screens.append((left, top, left + box["w"], top + box["h"]))
    # 只有包含本组合框已验证选项的唯一 owned 窗才是视口，不借用无关兄弟弹窗。
    matches = [popup for popup in popups if any(contains(popup["rect"], screen) for screen in screens)]
    if len(matches) > 1:
        raise FormControlReadError("form_control_popup_ambiguous")
    popup = matches[0] if matches else None
    viewport = (window[0], window[1], window[0] + window[2], window[1] + window[3])
    if diagnostics is not None:
        diagnostics.update(viewport_source="native_owned_popup" if popup else "bound_window",
                           viewport_excluded_count=0)
    associated = []
    for option, screen in zip(options, screens):
        if popup and (not contains(popup["rect"], screen) or not contains(viewport, screen)):
            if diagnostics is not None:
                diagnostics["viewport_excluded_count"] += 1
            continue
        if not contains(viewport, screen):
            raise FormControlReadError("form_control_geometry_unavailable")
        item = dict(option)
        if popup:
            ...
            item["native_popup"] = {**popup, "contract_version": "form_option_native_popup_v1",
...
def _dropdown(node, walker, wrap, handle, pid, window, diagnostics=None, *, expanded=None, root=None):
    ...
    for item in internal:
        ...
        if not _explicit_true_call(item, "is_visible") or not _explicit_true_call(item, "is_enabled"):
            continue
        before = _describe(item, handle, pid, window, allow_outside_window=expanded is True)
