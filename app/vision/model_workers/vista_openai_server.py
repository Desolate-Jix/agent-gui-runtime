...
def _activate_request(
    *,
    ...
    with ACTIVE_REQUEST_LOCK:
        if normalized_request_id in CANCELLED_REQUEST_IDS:
            raise ModelRequestCancelled(f"request {normalized_request_id} was cancelled before admission")
...
def _cancel_active_request(request_id: str) -> dict[str, Any]:
    ...
    with ACTIVE_REQUEST_LOCK:
        ...
        if not active_request_id:
            CANCELLED_REQUEST_IDS.add(normalized_request_id)
            return {
                "contract_version": "model_request_cancel_response_v1",
...
def _message_text_and_image(messages: list[dict[str, Any]]) -> tuple[str, Any | None]:
    ...
    for message in messages:
        # 定位任务只来自用户图文，传输格式说明和助手历史不能混成视觉目标。
...
def _vista_prompt(instruction: str) -> str:
    ...
    # 保留调用方的目标类型约束，不能把“可编辑区域”退化为同名标签定位。
    constraints = re.findall(r"(?im)^\s*Target constraints\s*:\s*(.+?)\s*$", cleaned) if goal_match else []
    if constraints:
        target_instruction += ". " + " ".join(item.strip() for item in constraints)
        target_instruction = target_instruction.rstrip(" .")
    return (
...
def _requested_coordinate_space(instruction: str) -> str:
    ...
    # 完整匹配坐标标识，避免 0_1 误命中 0_1000。
    if re.search(r"(?<![a-z0-9_])roi_local_point(?![a-z0-9_])", text):
        return "roi_local_point"
    if re.search(r"(?<![a-z0-9_])normalized_0_1(?![a-z0-9_])", text):
...
def _inspect_runtime() -> dict[str, Any]:
    """仅验证 VISTA 冻结依赖可导入，不读取权重或初始化模型。"""
    dependency = "torch"
    try:
        import torch
        dependency = "transformers"
        import transformers
        dependency = "transformers.AutoProcessor"
        from transformers import AutoProcessor
        dependency = "transformers.AutoModelForImageTextToText"
        from transformers import AutoModelForImageTextToText
        dependency = "transformers.models.qwen3_5.Qwen3_5ForConditionalGeneration"
        from transformers.models.qwen3_5 import Qwen3_5ForConditionalGeneration
        dependency = "transformers.models.qwen3_vl.processing_qwen3_vl.Qwen3VLProcessor"
        from transformers.models.qwen3_vl.processing_qwen3_vl import Qwen3VLProcessor
        dependency = "transformers.models.qwen3_vl.video_processing_qwen3_vl.Qwen3VLVideoProcessor"
        from transformers.models.qwen3_vl.video_processing_qwen3_vl import Qwen3VLVideoProcessor
        dependency = "transformers.models.qwen2_vl.image_processing_qwen2_vl_fast.Qwen2VLImageProcessorFast"
        from transformers.models.qwen2_vl.image_processing_qwen2_vl_fast import Qwen2VLImageProcessorFast
    except Exception as exc:
        return {
            "contract_version": "vista_runtime_check_v1",
            "status": "runtime_imports_failed",
    ...
    return {
        "contract_version": "vista_runtime_check_v1",
        "status": "runtime_imports_verified",
        "versions": {"torch": torch.__version__, "transformers": transformers.__version__},
        "imports": {
            "AutoProcessor": f"{AutoProcessor.__module__}.{AutoProcessor.__name__}",
            "AutoModelForImageTextToText": f"{AutoModelForImageTextToText.__module__}.{AutoModelForImageTextToText.__name__}",
            "Qwen3_5ForConditionalGeneration": f"{Qwen3_5ForConditionalGeneration.__module__}.{Qwen3_5ForConditionalGeneration.__name__}",
            "Qwen3VLProcessor": f"{Qwen3VLProcessor.__module__}.{Qwen3VLProcessor.__name__}",
            "Qwen3VLVideoProcessor": f"{Qwen3VLVideoProcessor.__module__}.{Qwen3VLVideoProcessor.__name__}",
            "Qwen2VLImageProcessorFast": f"{Qwen2VLImageProcessorFast.__module__}.{Qwen2VLImageProcessorFast.__name__}",
        },
        "model_loaded": False,
        "inference_verified": False,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check_runtime:
        ...
        return 0 if report["status"] == "runtime_imports_verified" else 2
    if not args.model_path:
        parser.error("--model-path is required unless --check-runtime")
