...
MANAGED_LEARNING_WORKFLOW_OPERATION_STAGES = frozenset({
    "screen_understanding",
    "numbered_map",
    "precise_calibration",
    "review_repair",
...
class LearningWorkflowManagedStageOperationRequiredError(LearningWorkflowStageOperationError):
    """受管阶段只能通过签发、续租和完成等专用操作推进。"""
...
def _direct_transition_requires_managed_operation(
    *,
    ...
    """只读判断可达的受管迁移；无效请求仍由原校验链解释。"""

    if (
        stage not in MANAGED_LEARNING_WORKFLOW_OPERATION_STAGES
        or not isinstance(expected_revision, int)
        or expected_revision < 0
        or not isinstance(current, dict)
        or current.get("revision") != expected_revision
        or current.get("terminal") is True
    ):
        return False
    current_stage = current.get("current_stage")
    current_status = current.get("stages", {}).get(current_stage, {}).get("status")
    if current_stage == stage:
        return current_status == "running" and outcome in {
            "running", "completed", "failed", "safe_stopped"
        }
    return (
        current_stage in LEARNING_WORKFLOW_STAGES
        and LEARNING_WORKFLOW_STAGES.index(stage)
        == LEARNING_WORKFLOW_STAGES.index(current_stage) + 1
...
def transition_learning_workflow_run(
    *,
    ...
    """公开迁移入口：受管策略在任何证据或状态写入之前执行。"""

    if stage in MANAGED_LEARNING_WORKFLOW_OPERATION_STAGES:
        try:
            current = store.get(run_id)
        except LearningWorkflowTransitionError:
            current = None
        if _direct_transition_requires_managed_operation(
            current=current, expected_revision=expected_revision,
            stage=stage, outcome=outcome,
        ):
            raise LearningWorkflowManagedStageOperationRequiredError(
                f"{stage} must use a managed workflow stage operation"
            )
    return _transition_learning_workflow_run_internal(
        store=store, project_root=project_root, run_id=run_id,
        expected_revision=expected_revision, stage=stage, outcome=outcome,
        reason=reason, evidence_refs=evidence_refs,
    )


def _transition_learning_workflow_run_internal(
    *,
    store: LearningWorkflowRunStore,
    project_root: str | Path,
    run_id: str,
...
def start_learning_workflow_stage_operation(
    *,
    ...
    state = _transition_learning_workflow_run_internal(
        store=store,
        project_root=project_root,
        run_id=run_id,
        expected_revision=expected_revision,
...
def _recover_benchmark_v2_incumbent_pre_reservation_workflow_service(
    *,
    ...
    with get_learning_workflow_operation_lock(
        ...
        recovered = _transition_learning_workflow_run_internal(
            store=composition.store,
            project_root=composition.project_root,
            run_id=run_id,
            expected_revision=int(current["revision"]),
...
def _attest_benchmark_v2_actual_completed_hybrid_cleanup(
    *,
    composition: LearningWorkflowServiceComposition,
    operation_ref: Mapping[str, object],
) -> dict[str, Any]:
    from app.learn.hybrid.benchmark_v2_incumbent_operation import (
        compose_benchmark_v2_actual_completed_hybrid_cleanup,
        validate_benchmark_v2_workflow_service_operation_ref,
    )
    ...
    with get_learning_workflow_operation_lock(
        ...
        if (
            ...
            raise LearningWorkflowStageOperationError(
                "benchmark_v2 completed Hybrid cleanup bridge is unavailable"
            )
        ...
        if (
            ...
            or exact["worker_ref"] != returned_worker_ref
        ):
            raise LearningWorkflowStageOperationError(
                "benchmark_v2 completed Hybrid continuation lineage is stale"
            )
        ...
        if (
            ...
            raise LearningWorkflowStageOperationError(
                "benchmark_v2 completed Hybrid worker absence is indeterminate"
            )
        cancellation_projection = seal_immutable(
            {
                "contract_version": (
                    "benchmark_v2_completed_hybrid_cancellation_projection_v1"
                ),
                "run_id": returned_worker_ref["run_id"],
                "stage": returned_worker_ref["stage"],
                "operation_id": returned_worker_ref["operation_id"],
                "worker_id": returned_worker_ref["worker_id"],
                "model_request_id": returned_worker_ref["model_request_id"],
                "payload_sha256": returned_worker_ref["payload_sha256"],
        ...
        return compose_benchmark_v2_actual_completed_hybrid_cleanup(
            operation_ref=exact,
            worker_cleanup_ref=worker_cleanup_ref,
...
def _ensure_benchmark_v2_incumbent_child_workflow_run(
    *,
    ...
    try:
        ...
        if str(error) != "workflow run not found":
            raise
        current = _transition_learning_workflow_run_internal(
            store=composition.store,
            project_root=composition.project_root,
            run_id=run_id,
            expected_revision=0,
...
def _benchmark_v2_hybrid_terminal_worker_cleanup_ref(
    *,
    stage_execution: Mapping[str, object],
    worker_record: Mapping[str, object],
) -> dict[str, Any] | None:
    ...
    return seal_immutable(
        {
            ...
            "backend_compute_termination": str(
                cancellation.get("backend_compute_termination") or "not_covered"
            ),
            "model_service_compute_termination": str(
...
def _benchmark_v2_hybrid_terminal_worker_cleanup_is_verified(
    *,
    ...
    cancellation_cleanup_verified = (
        ...
        and worker_cleanup.get("content_sha256") == content_sha256(worker_cleanup)
        and all(
...
def _benchmark_v2_hybrid_qwen_zero_dispatch_cleanup_verified(
    *,
    ...
    context_refs = binding.get("provider_dispatch_context_refs")
    context_ref_value = (
        context_refs.get("qwen") if isinstance(context_refs, Mapping) else None
    )
    window_binding = binding.get("window_binding")
    if not isinstance(context_ref_value, Mapping) or not isinstance(
    ...
    lineage_valid = (
        ...
        and operation_ref.get("capture_ref") == window_binding.get("capture_ref")
        and context.get("receipt_journal_path") == expected_journal_path
    )
...
def _benchmark_v2_hybrid_service_context(
    *,
    ...
    if current_status == "safe_stopped":
        ...
        recovered_cleanup_verified = isinstance(recovered_cleanup, Mapping)
        if not recovered_cleanup_verified:
            worker_cleanup_ref = _benchmark_v2_hybrid_terminal_worker_cleanup_ref(
                stage_execution=stage_execution,
                worker_record=worker_record,
            )
            ...
            recovered_cleanup_verified = (
                _benchmark_v2_hybrid_qwen_zero_dispatch_cleanup_verified(
                    composition=composition,
                    binding=binding,
                    worker_record=worker_record,
                    worker_cleanup_ref=worker_cleanup_ref,
