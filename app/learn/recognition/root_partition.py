    ...
    if not boxes:
        reliable_y = next(
            (
                cut
                for cut in y_cuts
                if float(cut.get("score") or 0.0) >= 0.8
                and float(cut.get("gap_ratio") or 0.0) >= 0.015
            ),
            None,
        )
        if reliable_y is not None:
            boxes = _cells_from_cuts(screen, "y", [reliable_y])
            source = "supported_horizontal_whitespace_cut"
