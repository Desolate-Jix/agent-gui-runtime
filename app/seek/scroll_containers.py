...
def discover_seek_scroll_containers(
    *,
    ...
    width = size["width"]
    height = size["height"]
    body_top = _clamp(int(round(height * 0.29)), 220, max(220, height - 220))
    body_bottom_margin = 16
    body_height = max(140, height - body_top - body_bottom_margin)

    results_x = 16
    results_width = _clamp(int(round(width * 0.36)), 360, min(480, max(360, int(round(width * 0.43)))))
    if results_x + results_width > width - 260:
        results_width = max(220, width // 2 - 32)
    detail_x = min(width - 220, results_x + results_width + 48)
    detail_width = max(220, width - detail_x - 24)
    page = {
