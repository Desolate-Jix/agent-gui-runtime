...
def list_traces(limit: int = 50) -> APIResponse:
...
def inspect_trace(path: str) -> APIResponse:
    ...
    try:
        ...
        # Capture info
        parsed["sections"]["capture"] = trace.get("live_capture") or trace.get("capture") or {"image_path": trace.get("image_path", "")}
        ...
        # OCR result
        ocr = trace.get("parse_result", {}).get("ocr_result") or trace.get("ocr_result") or {}
        parsed["ocr_count"] = _ocr_count(ocr)
        parsed["sections"]["ocr"] = ocr

        # Vision
        parsed["sections"]["vision"] = {
            "vision_regions": trace.get("parse_result", {}).get("vision_regions") or trace.get("vision_regions"),
        ...
        if isinstance(plan, dict) and plan:
            ...
            parsed["sections"]["candidates"] = candidate_result

            pre_click = plan.get("pre_click_decision") or trace.get("pre_click_decision") or {}
            parsed["gate_allowed"] = pre_click.get("allowed")
            parsed["gate_reason"] = str(pre_click.get("reason") or "")[:200]
            parsed["sections"]["gate"] = pre_click
