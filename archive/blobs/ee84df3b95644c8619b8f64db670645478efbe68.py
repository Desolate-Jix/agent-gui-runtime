...
class WindowsUIAProvider:
    ...
    def _browser_chrome_scope(self, root, *, bound, window):
        ...
        try:
            wrappers = list(islice(walk(root), budget))
            observed = []
            zero_area_ids = set()
            documents = 0
            for index, wrapper in enumerate(wrappers):
            ...
            return {"provider": self.provider_id, "provider_version": self.version,
                    ...
                    "scan_visited_count": len(wrappers), "excluded_document_count": documents,
                    "scan_complete": len(wrappers) < budget, "truncated": len(wrappers) == budget,
                    "truncation_reason": "control_budget_reached" if len(wrappers) == budget else None,
