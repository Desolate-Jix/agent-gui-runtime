...
def uia_action_identity_matches(control, *, goal, control_target=None, target_text=None):
    ...
    if marker is not None:
        # 只采用标签标记之前的直接目标角色，不使用标签或后续上下文提到的角色。
        role = re.search(r"\b(menu\s+item|radio\s+button|hyperlink|link|button|tab|checkbox|input|field)\s*$",
                         goal[:marker.start()], re.I)
        if role is not None:
            ...
            requested_role = " ".join(role.group(1).casefold().split())
