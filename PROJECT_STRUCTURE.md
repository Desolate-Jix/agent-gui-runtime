...
The latest SEEK contract smoke keeps quality and safety separate: `seek_mvp_accuracy_summary_v1.status` now requires both zero-submit safety and quality invariants such as opened/read coverage, no post-click layout drift, and no wrong-scope scrolls. Apply / Quick Apply route detection in `app/seek/application.py` can classify a visible same-site `/job/.../apply` route as `application_flow_opened`, but `build_seek_apply_flow_decision()` returns `wait_for_form_readiness` for that route-only state. `scripts/seek_mvp_traversal_runner.py` records `seek_application_form_readiness_wait_v1` and only allows downstream cover-letter / answer-plan / safe-fill stages after real form evidence appears.
`seek_mvp_accuracy_summary_v1` also treats `title_extraction_from_body_count>0` as a quality failure. This keeps scrolled-body fragments from masquerading as a stable detail header after a card click.
...
- SEEK scroll containers are now evidence-first. `extract_seek_job_cards` infers `seek:results_list` from current `screen_inventory_v1.cards` job-card bboxes and passes that bbox into `/action/scroll`; the centered wide-window layout is only a fallback. The traversal runner stops with `blocked_no_initial_job_cards` or `blocked_no_new_jobs_after_results_scroll` rather than blindly scrolling to the footer, and `scripts/stop_seek_mvp_runner.ps1` can stop a stuck SEEK runner without killing runtime/model services.
- SEEK execution timing and nested scrolling are calibrated in the runner/action boundary. Real card clicks use a `200ms` post-move settle before mouse-down, and `scripts/seek_mvp_traversal_runner.py` escalates repeated results/detail scrolls from `4` to `8` to `12` wheel clicks when the visible content fingerprint is not changing enough. After a card is opened, detail reading is recorded as `fixed_seek_job_detail_container_after_card_click` and uses the known `seek:job_detail` container rather than another precise target relocation. Matching outputs include `fit_summary` and `recommended_next_action`; Apply Entry is modeled as a form-flow entrance, while true final submit remains blocked by state and action guards. The debug runner no longer sleeps a fixed 6 seconds after Apply Entry: it emits `seek_application_flow_wait_v1`, reuses an already-classified application-flow state immediately, and only polls whole-screen observe up to the configured 3-second maximum when the Apply result is unclear.
- SEEK detail scrolling stops on right-pane no-progress evidence. `_read_detail_until_complete()` passes the current `seek:job_detail` bbox into `/action/scroll`, fingerprints the extracted detail content after each scroll, and stops with `right_detail_no_progress_after_scroll` when the right pane does not change, so bottomed-out detail scrolling does not continue into the left results list.
- `scripts/seek_debug_step_runner.py` is the live SEEK debug harness. It is intentionally single-step: each invocation performs one operation (`open`, `bind_and_resize_verify`, `extract_cards`, `dry_run_card`, `execute_card`, `verify_detail`, `read_detail_batch`, `read_detail_scroll`, or `match`), writes `seek_debug_step_state_v1`, stores a `seek_debug_step_report_v1` under `logs/smoke/seek_debug_step_run_latest/`, and exits. Debug `open` uses a separate browser window, and close-old-window handling skips multi-tab browser windows rather than closing unrelated tabs. After `verify_detail`, the default recommended long-read step is `read_detail_batch`; `read_detail_scroll` remains available for single-scroll diagnosis and validates right-detail movement plus left-results screenshot-crop stability. This is the required path for debugging screenshots, window-size drift, card click timing, and nested right-pane scroll scope before returning to the full traversal runner.
- `scripts/seek_long_read_benchmark_report.py` compares old `read_detail_scroll` step reports with new `read_detail_batch` reports. It emits `seek_long_read_benchmark_report_v1` with step/capture counts, unique OCR line counts, wrong-scope counts, optional timing totals, and a `recommended_path` decision so long-read speed improvements are measured instead of guessed from a live demo.
...
- `POST /vision/observe_screen` now uses that learned SEEK runtime graph as the primary structure when the current page matches the SEEK search-results/detail surface. The returned `screen_map_v1` carries `learned_path_graph_resolution`, replaces generic `top_bar/primary_area` sections with `top_search_area/results_list/job_detail/job_card/detail_header/detail_body`, and exposes `learned_path_graph_available_actions` for the upper agent. Application-form states remain outside this graph and must use the separate SEEK application-flow artifacts.
...
- Learn Mode Artifact Replay now also includes a SEEK Application Evidence panel. It reads `seek_application_fill_record_v1`, `seek_application_final_review_audit_v1`, and `seek_application_flow_artifact_v1` through the existing read-only `/panel/file` route, then shows filled fields, cover-letter length, employer-question count, screenshot/trace counts, audit decision, final-submit safety counters, and whether the loaded sample is a safe review boundary.
...
- `scripts/learn_execute_checkpoint_report.py` is the Learn/Execute MVP checkpoint builder. It emits SEEK safe-validation, SEEK task-run replay, and SEEK station-internal application-flow checkpoint reports, builds `artifacts\skills\learned_skill_matrix_v1.json`, and writes `learn_execute_mvp_checkpoint_report_v1` so the panel/test harness can prove Execute Mode covers click, scroll, input, read, guarded-action skills, and the stop-before-submit application-flow artifact before learning a new site. The latest checked station-internal application-flow sample is `logs\smoke\learn_execute_mvp_checkpoint_92822270_20260620.json`, which uses the 92822270 artifact with `employer_question_count=4` and `final_submissions=0`.
- `scripts/learn_sample_readiness_gate.py` combines the Learn/Execute checkpoint and unified artifact replay regression into `learn_sample_readiness_gate_v1`. This is the hard gate before starting a new Learn Mode sample: five learned baselines must pass, common Execute skills must be covered, the SEEK application-flow checkpoint must keep final submit forbidden and safe-fill required, no write/final-submit counters may fire, and `artifacts\templates\learn_sample_template_v1.json` must exist.
...
- `seek_application_fill_record_v1`: auditable station-internal SEEK application-fill record exported from one-step debug reports; stores filled fields, screenshots, action/vision traces, cover letter, employer-question count, review screenshot, and zero-submit counters
- `seek_application_flow_artifact_v1`: non-authorizing Learn Mode evidence exported from a reviewed station-internal SEEK application fill; captures `choose_documents`, `answer_employer_questions`, `update_seek_profile`, and `review_and_submit`, and requires safe-fill verification before future replay
...
- `seek_application_flow_checkpoint_report_v1`: checkpoint report proving the learned station-internal SEEK application-flow artifact is complete, non-authorizing, final-submit-forbidden, and safe-fill-verification-required
- `seek_application_flow_replay_report_v1`: dry-run strict replay plan for the learned station-internal SEEK application flow; it proves the replay script has the right transition order and evidence requirements before live execution
...
- `app/seek/employer_questions.py`: SEEK employer-question inventory, profile-backed answer planning, and local answer-control ranking before live question filling
...
- `scripts/seek_debug_export_application_fill_record.py`: CLI for turning a one-step SEEK debug run into `seek_application_fill_record_v1` before final-review audit and application-flow artifact export
- `artifacts/templates/job_search_application_workflow_template_v1.json`: reusable non-authorizing job-search/application workflow template derived from SEEK, for future job-board learning samples
- `artifacts/skills/job_search_application_workflow_skill_v1.json`: learned orchestration skill artifact that composes job recording, detail reading, screening, safe same-site application entry, form review reconciliation, and final-submit blocking
...
- `scripts/agent_trace_digest.py`: CLI for turning long trace JSON into compact `agent_trace_digest_v1` handoff summaries for ChatGPT or external agents, including request, model I/O, candidates, Gate, action result, and screenshot/overlay paths
...
- `AGENT_ONBOARDING.md` and `docs/AGENT_*`: external-agent onboarding docs for safe API usage, Learn Mode, Execute Mode, trace debugging, and a copyable Chinese prompt template.
...
- SEEK 滚动容器现在是 evidence-first：`extract_seek_job_cards` 会先从当前 `screen_inventory_v1.cards` 里的职位卡片 bbox 推导 `seek:results_list`，并把这个 bbox 传给 `/action/scroll`；超宽窗口居中布局只是 fallback。Traversal runner 在首屏没有职位卡片或连续滚动没有新增职位时会返回 `blocked_no_initial_job_cards` / `blocked_no_new_jobs_after_results_scroll`，不会继续盲目滚到页脚；`scripts/stop_seek_mvp_runner.ps1` 可停止卡住的 SEEK runner，且不会杀 runtime 或模型服务。
- SEEK 执行节奏和嵌套滚动校准在 runner/action 边界：真实卡片点击会在鼠标移动后等待 `200ms` 再 mouse-down；`scripts/seek_mvp_traversal_runner.py` 会在 results/detail 可见内容指纹变化不足时，把后续滚动从 `4` wheel clicks 提升到 `8`、`12`。卡片打开后，详情读取会记录为 `fixed_seek_job_detail_container_after_card_click`，直接读已知的 `seek:job_detail` 容器，不再为右侧栏重新跑精准定位。匹配结果会输出 `fit_summary` 和 `recommended_next_action`；Apply Entry 被建模为申请表入口，真正最终提交继续被状态层和 action guard 阻断。
- SEEK 详情滚动会在右栏无进展时停止：`_read_detail_until_complete()` 会把当前 `seek:job_detail` bbox 传给 `/action/scroll`，每次滚动后比对抽取到的右栏详情指纹；如果右栏没有变化，就以 `right_detail_no_progress_after_scroll` 停止，避免右栏到底后继续滚动影响左侧卡片列表。
- `scripts/seek_debug_step_runner.py` 是真实 SEEK 调试专用入口。它刻意设计成单步执行：每次调用只执行一个操作（`open`、`bind_and_resize_verify`、`extract_cards`、`dry_run_card`、`execute_card`、`verify_detail`、`read_detail_batch`、`read_detail_scroll` 或 `match`），写入 `seek_debug_step_state_v1`，在 `logs/smoke/seek_debug_step_run_latest/` 下保存 `seek_debug_step_report_v1`，然后立即退出。Debug `open` 使用独立浏览器窗口；旧窗口关闭逻辑会跳过多标签浏览器窗口，避免误关无关标签。`verify_detail` 后默认推荐 `read_detail_batch`；`read_detail_scroll` 保留为单次滚动诊断，会同时验证右侧详情是否移动，以及左侧结果栏截图裁剪是否稳定，避免 OCR / 卡片抽取噪声掩盖或伪造滚动串栏。调试截图、窗口尺寸漂移、卡片点击时序和右栏嵌套滚动范围时，必须先走这个入口，再回到完整 traversal runner。
...
- `scripts/agent_trace_digest.py`
  - reads a trace JSON with UTF-8 / UTF-8-SIG compatibility and emits compact `agent_trace_digest_v1`
  - summarizes request fields, timings, model I/O previews, screen/path graph evidence, top candidates, `pre_click_decision_v1`, action verification, and image artifact paths
  - refuses over-large trace files by default with `status=skipped_large_trace` so upper agents do not load huge raw traces into context
...
- `app/seek/final_review.py`: read-only final Review extraction and reconciliation for station-internal applications. It emits `seek_final_review_extraction_v1` / `review_reconciliation_v1` and is the source of reusable `skill:review_before_submit_reconciliation` when exported through the application-flow artifact.
...
- `scripts/seek_debug_step_runner.py --step extract_final_review`: observes the current Review page, compares it against `application_fill_record.json`, writes `final_review_extraction.json`, and performs no submit action.
