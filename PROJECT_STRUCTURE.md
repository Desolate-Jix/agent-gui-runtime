...
- `scripts/seek_debug_step_runner.py` is the live SEEK debug harness. It is intentionally single-step: each invocation performs one operation (`open`, `bind_and_resize_verify`, `extract_cards`, `dry_run_card`, `execute_card`, `verify_detail`, `read_detail_scroll`, or `match`), writes `seek_debug_step_state_v1`, stores a `seek_debug_step_report_v1` under `logs/smoke/seek_debug_step_run_latest/`, and exits. Debug `open` uses a separate browser window, and close-old-window handling skips multi-tab browser windows rather than closing unrelated tabs. `read_detail_scroll` validates the right-detail movement and the left-results pane with screenshot-crop visual stability, so OCR/card extraction noise cannot hide or invent scroll bleed. This is the required path for debugging screenshots, window-size drift, card click timing, and nested right-pane scroll scope before returning to the full traversal runner.
...
- `scripts/seek_debug_step_runner.py` 是真实 SEEK 调试专用入口。它刻意设计成单步执行：每次调用只执行一个操作（`open`、`bind_and_resize_verify`、`extract_cards`、`dry_run_card`、`execute_card`、`verify_detail`、`read_detail_scroll` 或 `match`），写入 `seek_debug_step_state_v1`，在 `logs/smoke/seek_debug_step_run_latest/` 下保存 `seek_debug_step_report_v1`，然后立即退出。Debug `open` 使用独立浏览器窗口；旧窗口关闭逻辑会跳过多标签浏览器窗口，避免误关无关标签。`read_detail_scroll` 会同时验证右侧详情是否移动，以及左侧结果栏截图裁剪是否稳定，避免 OCR / 卡片抽取噪声掩盖或伪造滚动串栏。调试截图、窗口尺寸漂移、卡片点击时序和右栏嵌套滚动范围时，必须先走这个入口，再回到完整 traversal runner。
